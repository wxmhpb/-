...
public class SearchService {
    ...
    public SearchResult search(SearchRequest request) {
       ...
       // queryBuilder.withQuery(basicQuery);
        ...
        QueryBuilder boolQueryBuilder = this.buildBooleanQueryBuilder(request);
        ...
        // 封装成需要的返回结果集
        return new SearchResult(goodsPage.getContent(), goodsPage.getTotalElements(), Long.valueOf(goodsPage.getTotalPages()), categories, brands, specs);
    }
    /**
     * 解析品牌聚合结果集
     * @param aggregation
   ...
   private QueryBuilder buildBooleanQueryBuilder(SearchRequest searchRequest) {
       ...
       searchRequest.getFilter().forEach((key, val) -> {
           if ("品牌".equals(key)) {
               //品牌过滤brandId
               key = "brandId";
